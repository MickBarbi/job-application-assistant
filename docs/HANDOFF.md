...
1. Improve the completed app with richer resume-generation tests and deployment
   packaging.
...
1. **Successful generation-path tests:** add injected/fake OpenAI and PDF
   boundaries so `POST /api/resumes/generate` can be tested without network or
   LaTeX.
2. **Deployment packaging:** add a Dockerfile/deployment guide with a bundled
   LaTeX engine for reliable PDF export.
3. **Settings UX refinements:** consider reorder controls and richer validation
